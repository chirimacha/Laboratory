...
setwd("~/Desktop/Laboratory/infectChiriAnalysis/polygons")
...
# the arena. You supply the data frame with the track information of the bugs, and it will
...
getTotalDistance <- function(df) {
  ...
  # Here we have a function which will test whether the data frame has information
  ...
  # We now have a function that will first, create all of the frames that we need
  ...
  # We now return a data frame that adds up the previously described distance
  # column in order to use it for analysis
...
#===================================================================================
# Part IId. In the WALL area: total distance AND total time traveled by hour by the bugs (two results)
#===================================================================================
# The purpose of this function is to do two things. The first is to get the total time spent
# within the open area, and the second is to get the total distance traversed within the
# open area.
TimeMovingWALL <- function(df, inputPoly) {
  # Only keep columns we need
  toKeep <- c("frame", "track", "x", "y")
  df <- df[,toKeep]
...
# This claculates the total distance traveled from pixels to centimeters
...
df["seconds.time.spent.in.open"] <- df$time_spent_in_open / 29.9 * 8

# This calculates the distance spent in an open area from pixels to centimeters
df["open.distance.cm"] <- round(df$distance_spent_in_open * 0.02645833, 2)
...
output_direc <- "~/Desktop/Laboratory/infectChiriAnalysis/results/resultsByHour"
# hour 1
...
write.csv(df3, filename, row.names = F)
# main data frame
filename <- file.path(output_direc, paste("resultsByHour.csv", sep=""))
