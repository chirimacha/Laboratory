...
chiri.vid <- readVid("20160712cam1.mp4")
...
bg <- backgrounder(chiri.vid, n=150, method="median", color= TRUE)
...
mat1 <- matrix(1, nrow = bg$dim[1], ncol = bg$dim[2])
...
for (j in 1:383) {
  ...
  for (l in begin.frame:end.frame) {
    res <- getFrame(chiri.vid, l) # extract individual frames
